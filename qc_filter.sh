...
# MyRocks doesn't support generated columns
DIRLIST=$(grep -i "ERROR.*3106.*is not supported for generated columns" */pquery_thread-0.RocksDB.sql|awk -F '/' '{print $1}'|sort -u)
move

# MyRocks doesn't support native partitioning
DIRLIST=$(grep -i "Please use native partitioning instead." */pquery_thread-0.RocksDB.out|awk -F '/' '{print $1}'|sort -u)
move
