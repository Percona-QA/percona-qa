...
if [[ $BTYPE == 'debug' ]];then
    EXTRAOPTS+=" --mysqld=--skip-performance-schema "
fi
...
if ! grep -qw 'skip' <<< $REST;then

    pecho "Running with normal grammar "
    ttype="normal"
    CVARDIR=$(mktemp -d --tmpdir=$WORKDIR 'normal-XXX')
    timeout -s 11  $TTMT perl runall-new.pl --basedir=${MYSQL_BASEDIR} --vardir=$CVARDIR --galera=$GTYPE $EXTRAOPTS  --reporter=$RPTR --grammar=conf/galera/galera_stress-56.yy --gendata=conf/galera/galera_stress-56.zz --threads=$RUNT --queries=$RUND --mysqld=--wsrep-provider=$MYSQL_BASEDIR/lib/libgalera_smm.so --seed=time  --mysqld=--innodb_flush_method=O_DIRECT  || failed
    ...
    timeout -s 11  $TTMT perl runall-new.pl --basedir=${MYSQL_BASEDIR} --vardir=$CVARDIR --galera=$GTYPE $EXTRAOPTS  --reporter=$RPTR --grammar=conf/galera/galera_stress-complex-56.yy --gendata=conf/galera/galera_stress-complex.zz --threads=$RUNT --queries=$RUND --mysqld=--wsrep-provider=$MYSQL_BASEDIR/lib/libgalera_smm.so  --mysqld=--innodb_flush_method=O_DIRECT  --seed=time  || failed
...
if ! grep -qw 'skip2' <<< $REST;then
    ...
    --clean \
    --force \
    --config=conf/galera/galera_stress-56.cc \
    --grammar=conf/galera/galera_stress-56.yy \
    --gendata=conf/galera/galera_stress-56.zz \
    --seed=time \
    --trials=${RTRIALS:-10} \
