...
for X in $(seq 0 ${MYSQLD_START_TIMEOUT}); do
  sleep 1
  if ${BASEDIR}/bin/mysqladmin -uroot -S${PQUERY_PWD}/${TRIAL}/socket.sock ping > /dev/null 2>&1; then
    break
  fi
done
...
dbs=$(${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse "select schema_name from information_schema.schemata where schema_name not in ('mysql','information_schema','performance_schema')" )
# Check tables
for i in "${dbs[@]}"; do
  tbs=$(${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse "select table_name from information_schema.tables where table_schema='$i'")
  tbl_array=( $( for i in $tbs ; do echo $i ; done ) )
  for j in "${tbl_array[@]}"; do
    echo -e "\nVerifying table: $j; database: $i\n"
    chk_result=$((${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse $i "CHECK TABLE $j EXTENDED") 2>&1)
    analyze_result=$((${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse $i "ANALYZE TABLE $j") 2>&1)
    opt_result=$((${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse $i "OPTIMIZE TABLE $j") 2>&1)
    repair_result=$((${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse $i "REPAIR TABLE $j EXTENDED") 2>&1)
    alter_result=$((${BASEDIR}/bin/mysql --socket=${PQUERY_PWD}/${TRIAL}/socket.sock -uroot -Bse $i "ALTER TABLE $j ENGINE = TokuDB") 2>&1)
    ERROR_INFO=""
