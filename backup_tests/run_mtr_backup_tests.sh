...
download_tarballs() {
    ...
    if [ "$1" = "pxb80" ]; then
        ...
        ps_tarball_dir="${test_home_dir}/Percona-Server-${PS80_VER}-Linux.x86_64.glibc2.17"
        ...
        ps_tarball_dir="${test_home_dir}/Percona-Server-${PS57_VER}-Linux.x86_64.glibc2.17"
    fi

    echo "Unpacking PXB binary tarball" | tee -a "${log}"
    tar -xf ${pxb_tarball_dir}.tar.gz
    mv ${pxb_tarball_dir} ${product}
    pxb_tarball_dir=${product}

    echo "Unpacking Percona Server binary tarball" | tee -a "${log}"
    tar -xf ${ps_tarball_dir}.tar.gz
    mv ${ps_tarball_dir} percona_server
