...
PLUGINS_INIT_FILE=${SCRIPT_PWD}/plugins_56.sql                       # Only used when matching MYEXTRA is used below (with --init-file=${PLUGINS_INIT_FILE}). Set correct version
...
#MYEXTRA="--plugin-load-add=audit_log=audit_log.so --plugin-load-add=tokudb=ha_tokudb.so --init-file=${PLUGINS_INIT_FILE} --tokudb-check-jemalloc=0"  # PS: All plugins
