...
BASEDIR=/sda/PS-mysql-5.7.9-90.0-linux-x86_64-debug           # Basedir. Note that even for PXC runs, a PS basedir is needed (we need mysql client, mysqladmin,...)
...
pquery_test(){
  ...
  if [ ${PXC} -eq 0 ]; then
    ...
    echo "${CMD//$RUNDIR/$WORKDIR}  > ${WORKDIR}/${TRIAL}/log/master.err 2>&1 &" >> ${RUNDIR}/${TRIAL}/start_recovery ; chmod +x ${RUNDIR}/${TRIAL}/start_recovery
