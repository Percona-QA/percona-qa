...
WORKDIRACTIVE=0; SAVED=0; TRIAL=0; MYSQLD_START_TIMEOUT=60
...
TOKUDB_OPTION_INFILE=${SCRIPT_PWD}/pquery/mysqld_options_tokudb.txt # TokuDB options list
                                                                    # mysqld_options_ps_56.txt, mysqld_options_ms_56.txt, mysqld_options_ms_57.txt, mysqld_options_ps_55.txt etc.
BASEDIR=/sda/PS-mysql-5.7.9-1-linux-x86_64-debug                    # Basedir. Note that even for PXC runs, a PS basedir is needed (we need mysql client, mysqladmin,...)
...
#MYEXTRA="--plugin-load=audit_log=audit_log.so;tokudb=ha_tokudb.so --init-file=${SCRIPT_PWD}/TokuDB.sql"                              # Use for TokuDB & audit-log testing
