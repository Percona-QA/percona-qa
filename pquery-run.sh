...
BASEDIR=/sda/FB-04.05.16-mysql-5.6.27-linux-x86_64                   # Basedir. Note that even for PXC runs, a PS basedir is needed (we need mysql client, mysqladmin,...)
...
#MYEXTRA="--plugin-load-add=audit_log=audit_log.so --plugin-load-add=tokudb=ha_tokudb.so --init-file=${SCRIPT_PWD}/plugins.sql --tokudb-check-jemalloc=0"  # PS: All plugins
