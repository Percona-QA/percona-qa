pipeline {
  ...
  parameters {
    ...
    string(name: 'BRANCH', defaultValue: '5.6', description: 'Target branch')
  ...
  stages {
    stage('Build PS binary') {
      steps {
        ...
        copyArtifacts filter: 'public_url', fingerprintArtifacts: true, projectName: 'percona-server-5.6-pipeline', selector: specific("${PIPELINE_BUILD_NUMBER}")
        script {
