...
####################################################
#  Script to start PXC server with keyring_vault   #
#  Kill and restart                                #
####################################################
BASEDIR=$(realpath $1)
...
${WORKDIR}/vault_test_setup.sh --workdir=$BASEDIR/pxc-node/vault --setup-pxc-mount-points --use-ssl > /dev/null 2>&1
...
loose-keyring_vault_config=$BASEDIR/pxc-node/vault/keyring_vault_pxc1.cnf
...
plugin_dir=$BASEDIR/lib/plugin
early-plugin-load=keyring_vault.so
loose-keyring_vault_config=$BASEDIR/pxc-node/vault/keyring_vault_pxc2.cnf
...
plugin_dir=$BASEDIR/lib/plugin
early-plugin-load=keyring_vault.so
loose-keyring_vault_config=$BASEDIR/pxc-node/vault/keyring_vault_pxc3.cnf
...
pxc_startup_status(){
  ...
  for X in $(seq 0 ${PXC_START_TIMEOUT}); do
    ...
    if [ $X -eq ${PXC_START_TIMEOUT} ]; then
      echo "Node$NR failed to start. Check logs"
      exit 1
    fi
  done
}
...
echo "Inserting data"
${BASEDIR}/bin/mysql -uroot -S${SOCKET1} -e"CREATE DATABASE test;"
${BASEDIR}/bin/mysql -uroot -S${SOCKET1} -e"CREATE TABLE test.t1(a int) ENCRYPTION='Y';"
#${BASEDIR}/bin/mysql -uroot -S${SOCKET1} -e"CREATE TABLESPACE tab02k_e ADD DATAFILE 'tab02k_e.ibd'  FILE_BLOCK_SIZE 02k ENCRYPTION='Y';"
...
echo "Copying the data directories"
cp -R $BASEDIR/pxc-node/dn1 $WORKDIR/dn1
cp -R $BASEDIR/pxc-node/dn2 $WORKDIR/dn2
cp -R $BASEDIR/pxc-node/dn3 $WORKDIR/dn3

cp ${WORKDIR}/dn1/*.pem ${WORKDIR}/cert/
cp ${WORKDIR}/dn2/*.pem ${WORKDIR}/cert/
cp ${WORKDIR}/dn3/*.pem ${WORKDIR}/cert/

sed -i 's|safe_to_bootstrap:.*$|safe_to_bootstrap: 1|' $WORKDIR/dn1/grastate.dat

echo "Starting server, perform crash recovery"
