...
cat > "$BASEDIR/lib/plugin/component_keyring_file.cnf" << EOF
{
...
cat > $WORKDIR/dn1/component_keyring_file.cnf << EOF
{
 "path": "${WORKDIR}/dn1/component_keyring_file",
 "read_only": false
}
EOF

echo "Create Local config for node2"
cat > $WORKDIR/dn2/component_keyring_file.cnf << EOF
{
 "path": "${WORKDIR}/dn2/component_keyring_file",
 "read_only": false
}
EOF

echo "Create Local config for node3"
cat > $WORKDIR/dn3/component_keyring_file.cnf << EOF
{
 "path": "${WORKDIR}/dn3/component_keyring_file",
 "read_only": false
}
EOF
