query:
  { $idx_table = '' ; @idx_fields = () ; @idx_select_fields = () ; "" } query_type ;
...
# We use the "AS table" bit here so we can have unique aliases if we use the same table many times
       { my @table_set = ("C","C","C","C","CC","CC","CC","CC","B","BB","B","BB","D") ; $stack->push(); my $x = $prng->arrayElement(\@table_set)." AS table".++$tables;  my @s=($x); $stack->pop(\@s); $x } ;
