...
#BASEDIR=/sda/PS190716-percona-server-5.7.13-6-linux-x86_64-debug     # MySQL Basedir. Required for all runs, including PXC 
#BASEDIR=/sda/mysql-5.7.13-linux-glibc2.5-x86_64     # MySQL Basedir. Required for all runs, including PXC 
BASEDIR=/sda/PS130916-percona-server-5.6.31-77.0-linux-x86_64-debug
...
# ========================================= User configurable variablesi: generics 2 ===============================================
...
PQUERY_RUN_TIMEOUT=600                                         # x sec max trial runtime within which pquery tries to process QUERIES_PER_THREAD x THREADS queries against 1 mysqld
...
#MYEXTRA="--slow_query_log --long_query_time=0 --log_slow_verbosity=full --log_slow_rate_type=query --log_slow_rate_limit=100 --slow_query_log_use_global_control=all"
#MYEXTRA="--slow_query_log --long_query_time=0"
MYEXTRA="--performance-schema --performance-schema-instrument='%=on'"
...
#MYEXTRA="--thread_handling=pool-of-threads --plugin-load-add=tokudb=ha_tokudb.so --init-file=${SCRIPT_PWD}/plugins_56.sql --tokudb-check-jemalloc=0" # PS 5.6: All plugins
#MYEXTRA="--thread_handling=pool-of-threads --plugin-load-add=tokudb=ha_tokudb.so --init-file=${SCRIPT_PWD}/plugins_57.sql --tokudb-check-jemalloc=0" # PS 5.7: All plugins
...
#MYSAFE="--no-defaults --event-scheduler=ON --maximum-bulk_insert_buffer_size=1M --maximum-join_buffer_size=1M --maximum-max_heap_table_size=1M --maximum-max_join_size=1M --maximum-myisam_max_sort_file_size=1M --maximum-myisam_mmap_size=1M --maximum-myisam_sort_buffer_size=1M --maximum-preload_buffer_size=1M --maximum-query_alloc_block_size=1M --maximum-query_prealloc_size=1M --maximum-range_alloc_block_size=1M --maximum-read_buffer_size=1M --maximum-read_rnd_buffer_size=1M --maximum-sort_buffer_size=1M --maximum-tmp_table_size=1M --maximum-transaction_alloc_block_size=1M --maximum-transaction_prealloc_size=1M --log-output=none --sql_mode=ONLY_FULL_GROUP_BY" #5.5
...
pquery_test(){
  ...
  if [ ${PXC} -eq 0 ]; then
    ...
    if ${BASEDIR}/bin/mysqladmin -uroot -S${RUNDIR}/${TRIAL}/socket.sock ping > /dev/null 2>&1; then
      ...
      #sudo pmm-admin add mysql pq${RANDOMD}-${TRIAL} --socket=${RUNDIR}/${TRIAL}/socket.sock --user=root --query-source=slowlog
  ...
  if [ ${ISSTARTED} -eq 1 ]; then  # Do not try and print pquery log for a failed mysqld start
    if [ ${QUERY_CORRECTNESS_TESTING} -eq 1 ]; then
      echoit "Pri engine pquery run details: $(grep -i 'SUMMARY.*queries failed' ${RUNDIR}/${TRIAL}/*.sql ${RUNDIR}/${TRIAL}/*.log)"
      echoit "Sec engine pquery run details: $(grep -i 'SUMMARY.*queries failed' ${RUNDIR}/${TRIAL}/*.sql ${RUNDIR}/${TRIAL}/*.log)"
    else
      echoit "pquery run details: $(grep -i 'SUMMARY.*queries failed' ${RUNDIR}/${TRIAL}/*.sql ${RUNDIR}/${TRIAL}/*.log)"
    fi
  fi
