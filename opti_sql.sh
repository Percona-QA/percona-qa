...
rm -f optisql.sql.tmp
touch optisql.sql.tmp
grep --binary-files=text -iE "^CREATE TABLE " new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^DROP TABLE "   new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^INSERT INTO "  new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^UPDATE "       new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^DELETE FROM "  new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^ALTER TABLE "  new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^TRUNCATE "     new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^SELECT "       new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
grep --binary-files=text -iE "^LOCK TABLE"    new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
#grep --binary-files=text -iE "^EXPLAIN "      new-main-md.sql | sed -e 's|t[2-9]|t|g' >> optisql.sql.tmp
cat optisql.sql.tmp | \
  grep --binary-files=text -viE "CHARSET|CHARACTER|utf|latin|ucs|hp|bin|cp|ascii|sjis|ujis|ci|\@|JSON" | \
  ...
  sed -e 's|[ ]\+| |g' \
 > optisql.sql   # ^ Note there is no pipe here
rm -f optisql.sql.tmp
