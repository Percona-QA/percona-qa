...
$OLD_INSTALL_DIR/bin/psql -p "$OLD_PORT" -d postgres -h "$PGHOST" -c "CREATE TABLE ghost_table (a int) USING tde_heap;"
$OLD_INSTALL_DIR/bin/psql -p "$OLD_PORT" -d postgres -h "$PGHOST" -c "INSERT INTO ghost_table VALUES (1),(2);"
$OLD_INSTALL_DIR/bin/psql -p "$OLD_PORT" -d postgres -h "$PGHOST" -c "DROP TABLE ghost_table;"
$OLD_INSTALL_DIR/bin/psql -p "$OLD_PORT" -d postgres -h "$PGHOST" -c "CREATE TABLE ghost_table (a int) USING tde_heap;"
$OLD_INSTALL_DIR/bin/psql -p "$OLD_PORT" -d postgres -h "$PGHOST" -c "VACUUM FULL"
$OLD_INSTALL_DIR/bin/psql -p "$OLD_PORT" -d postgres -h "$PGHOST" -c "INSERT INTO ghost_table VALUES (13),(24)"

ROW_COUNT_BEFORE=$("$OLD_INSTALL_DIR/bin/psql" -p "$OLD_PORT" -d postgres -h "$RUN_DIR" -t -c "SELECT count(*) FROM ghost_table;" | tr -d ' ')
...
$NEW_INSTALL_DIR/bin/psql -p "$NEW_PORT" -d postgres -h "$PGHOST" -c "SELECT * FROM ghost_table;"
if [ $? -ne 0 ]; then
    echo "❌ SELECT from ghost_table failed on new cluster"
    exit 1
fi

ROW_COUNT_AFTER=$("$NEW_INSTALL_DIR/bin/psql" -p "$NEW_PORT" -d postgres -h "$RUN_DIR" -t -c "SELECT count(*) FROM ghost_table;" | tr -d ' ')
...
if [ "$ROW_COUNT_AFTER" = "$ROW_COUNT_BEFORE" ]; then
    echo "[PASS] Row count verified: $ROW_COUNT_AFTER rows in ghost_table after upgrade"
else
