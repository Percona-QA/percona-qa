...
change_key_provider(){
    ...
    while [ $SECONDS -lt $end_time ]; do
        ...
        $INSTALL_DIR/bin/psql -d sbtest -c"SELECT pg_tde_add_key_provider_${provider_type}('$provider_name',$provider_config)"
        $INSTALL_DIR/bin/psql -d sbtest -c"SELECT pg_tde_set_principal_key('principal_key_sbtest','$provider_name')"
        sleep 1
    done
}
...
$INSTALL_DIR/bin/psql -d sbtest -c"SELECT pg_tde_set_global_principal_key('principal_key_sbtest','global_keyring');"
...
# Run sysbench for 30 seconds and 50 tables
run_sysbench_load 30 50 &
