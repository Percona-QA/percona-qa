...
# Cleanup
kill -9 $LOAD_PID $MAINT_PID $ADD_PID $DROP_PID $CREATE_PID $DROP_INDEX_PID
wait $LOAD_PID $MAINT_PID $ADD_PID $DROP_PID $CREATE_PID $DROP_INDEX_PID 2>/dev/null

if grep -q "Invalid page in block" "$LOG_FILE"; then
    echo "ERROR Found: Invalid page in block"
    exit 1
else
