...
pkill -9 -x pg_receivewal || true
rm -rf "${PGDATA}_bk" $BACKUP_DIR $KEYFILE || true
mkdir -p "$ARCHIVE_DIR"
...
"$INSTALL_DIR/bin/psql" -d postgres -p $PORT -c "CREATE TABLE t1(id SERIAL PRIMARY KEY, name TEXT);"
...
for i in {1..4}; do
    ...
    "$INSTALL_DIR/bin/psql" -d postgres -p $PORT -c "CREATE TABLE IF NOT EXISTS t_recovery_$i (id INT PRIMARY KEY, name TEXT);"
