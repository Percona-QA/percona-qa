...
$INSTALL_DIR/bin/psql -d $DB_NAME -c "SELECT pg_tde_add_database_key_provider_file('local_keyring','$PRIMARY_DATA/keyring.file');"
$INSTALL_DIR/bin/psql -d $DB_NAME -c "SELECT pg_tde_add_global_key_provider_file('global_keyring','$PRIMARY_DATA/keyring.file');"
...
start_pg $REPLICA_DATA $REPLCA_PORT
PID_REPLICA=$(lsof -ti :$REPLCA_PORT)
