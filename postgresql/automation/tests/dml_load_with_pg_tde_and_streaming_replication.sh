...
create_keys_and_insert_data() {
    ...
    sysbench /usr/share/sysbench/oltp_insert.lua --pgsql-user=`whoami` --pgsql-db=$DB_NAME --db-driver=pgsql --pgsql-port=$PRIMARY_PORT --threads=5 --tables=$TABLES --table-size=1000 prepare
    sysbench /usr/share/sysbench/bulk_insert.lua --pgsql-user=`whoami` --pgsql-db=$DB_NAME --db-driver=pgsql --pgsql-port=$PRIMARY_PORT --threads=5 --tables=$TABLES --table-size=1000
}
...
initialize_server $PRIMARY_DATA
...
if [ -d $REPLICA_DATA ]; then rm -rf $REPLICA_DATA; fi
...
echo "4=>Install pg_tde extension, create keys and Generate Load on Primary Server"
create_keys_and_insert_data
