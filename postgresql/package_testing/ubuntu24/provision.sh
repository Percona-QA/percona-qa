...
sudo apt-get update
sudo apt-get install -y percona-postgresql-$PG_VERSION

# Add pg_tde extension in shared_preload_libraries in the PGCONF and restart server
sudo sed -i -E "s|^\s*shared_preload_libraries\s*=\s*'[^']*'|shared_preload_libraries = 'pg_tde,percona_pg_telemetry'|" /etc/postgresql/$PG_VERSION/main/postgresql.conf
