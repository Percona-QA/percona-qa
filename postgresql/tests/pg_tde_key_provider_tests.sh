...
delete_key_provider_and_key() {
    db=$1
    # Delete key providers
    $INSTALL_DIR/bin/psql -d $db -c"SELECT pg_tde_delete_database_key_provider('vault_keyring');" > /dev/null
    $INSTALL_DIR/bin/psql  -d $db -c"SELECT pg_tde_delete_global_key_provider('vault_keyring');" > /dev/null
    $INSTALL_DIR/bin/psql  -d $db -c"SELECT pg_tde_delete_database_key_provider('kmip_keyring');" > /dev/null
    $INSTALL_DIR/bin/psql  -d $db -c"SELECT pg_tde_delete_global_key_provider('kmip_keyring');" > /dev/null
    $INSTALL_DIR/bin/psql  -d $db -c"SELECT pg_tde_delete_database_key_provider('file_keyring');" > /dev/null
    $INSTALL_DIR/bin/psql  -d $db -c"SELECT pg_tde_delete_global_key_provider('file_keyring');" > /dev/null
}
...
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_kmip('kmip_keyring','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_key_jane_doe.pem');"
$INSTALL_DIR/bin/psql  -d db1 -c"SELECT pg_tde_add_database_key_provider_vault_v2('vault_keyring','$token','$vault_url','$secret_mount_point','$vault_ca');"
...
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_vault_v2('vault_keyring2','$token','$vault_url','$secret_mount_point','$vault_ca');"
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_kmip('kmip_keyring2','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_key_jane_doe.pem');"
...
$INSTALL_DIR/bin/psql  -d test1 -c"SELECT pg_tde_add_database_key_provider_vault_v2('vault_keyring3','$token','$vault_url','$secret_mount_point','$vault_ca');"
echo "..Create a Principal key stored in vault for test1"
$INSTALL_DIR/bin/psql  -d test1 -c"SELECT pg_tde_set_key_using_database_key_provider('vault_key3','vault_keyring3','true');"

echo "..Add a Global Kmip key provider"
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_kmip('kmip_keyring3','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_key_jane_doe.pem');"

echo "..Create a Global Default Principal key stored in kmip"
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_set_default_key_using_global_key_provider('kmip_key3','kmip_keyring3','true');"
...
delete_key_provider_and_key test1
delete_key_provider_and_key test2
restart_server $data_dir
...
$INSTALL_DIR/bin/psql  -d sbtest2 -c"SELECT pg_tde_add_database_key_provider_vault_v2('vault_keyring4','$token','$vault_url','$secret_mount_point','$vault_ca');"
echo "..Create a Principal key stored in vault for sbtest2"
$INSTALL_DIR/bin/psql  -d sbtest2 -c"SELECT pg_tde_set_key_using_database_key_provider('vault_key4','vault_keyring4','false');"
...
$INSTALL_DIR/bin/psql  -d sbtest2 -c"SELECT pg_tde_add_database_key_provider_kmip('kmip_keyring4','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_certificate_jane_doe.pem','/tmp/certs/client_certificate_jane_doe.pem','/tmp/certs/client_key_jane_doe.pem');"
echo "..Create a Principal key stored in kmip for sbtest2"
$INSTALL_DIR/bin/psql  -d sbtest2 -c"SELECT pg_tde_set_key_using_database_key_provider('kmip_key4','kmip_keyring4','false');"
...
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_kmip('kmip_keyring5','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_key_jane_doe.pem');"
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_vault_v2('vault_keyring5','$token','$vault_url','$secret_mount_point','$vault_ca');"
...
$INSTALL_DIR/bin/psql  -d sbtest5 -c"SELECT pg_tde_change_global_key_provider_kmip('kmip_keyring5','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_key_jane_doe.pem');"
echo "..Create a Principal key stored in kmip for sbtest5"
$INSTALL_DIR/bin/psql  -d sbtest5 -c"SELECT pg_tde_set_key_using_global_key_provider('kmip_key5','kmip_keyring5');"
...
echo "..Change Key provider to vault"
$INSTALL_DIR/bin/psql  -d sbtest5 -c"SELECT pg_tde_change_global_key_provider_vault_v2('vault_keyring5','$token','$vault_url','$secret_mount_point','$vault_ca');"
echo "..Create a Principal key stored in vault for sbtest5"
$INSTALL_DIR/bin/psql  -d sbtest5 -c"SELECT pg_tde_set_key_using_global_key_provider('vault_key5','vault_keyring5');"
$INSTALL_DIR/bin/psql  -d sbtest5 -c"CREATE TABLE t3(a INT, b varchar) USING tde_heap;"
$INSTALL_DIR/bin/psql  -d sbtest5 -c"INSERT INTO t3 VALUES(300,'Percona');"
...
$INSTALL_DIR/bin/psql  -d sbtest5 -c"SELECT * FROM t3;"
...
echo "################################################################"
$INSTALL_DIR/bin/psql  -d postgres -c"CREATE DATABASE sbtest6"
$INSTALL_DIR/bin/psql  -d sbtest6 -c"CREATE EXTENSION pg_tde;"
$INSTALL_DIR/bin/psql  -d sbtest6 -c"SELECT pg_tde_add_database_key_provider_kmip('kmip_keyring6','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_certificate_jane_doe.pem','/tmp/certs/client_key_jane_doe.pem');"
$INSTALL_DIR/bin/psql  -d sbtest6 -c"SELECT pg_tde_set_key_using_database_key_provider('kmip_key6','kmip_keyring6');"
$INSTALL_DIR/bin/psql  -d sbtest6 -c"CREATE TABLE t1(a INT, b varchar) USING tde_heap;"
$INSTALL_DIR/bin/psql  -d sbtest6 -c"INSERT INTO t1 VALUES(100,'Mohit');"
...
$INSTALL_DIR/bin/psql  -d sbtest6 -c"INSERT INTO t2 VALUES(200,'Rohit');"
$INSTALL_DIR/bin/psql  -d sbtest6 -c"SELECT * FROM t2;"
...
echo "#################################################################"
$INSTALL_DIR/bin/psql  -d postgres -c"CREATE DATABASE db7;"
$INSTALL_DIR/bin/psql  -d db7 -c"CREATE EXTENSION pg_tde;"

$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_add_global_key_provider_vault_v2('keyring_vault7','$token','$vault_url','$secret_mount_point','$vault_ca');"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_set_default_key_using_global_key_provider('my_global_default_key1','keyring_vault7');"
$INSTALL_DIR/bin/psql  -d db7 -c"CREATE TABLE t1(a INT PRIMARY KEY, b VARCHAR) USING tde_heap;"
$INSTALL_DIR/bin/psql  -d db7 -c"INSERT INTO t1 VALUES(101, 'James Bond');"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT * FROM t1;"

echo "Rotate the Global Default Principal Key"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_set_default_key_using_global_key_provider('my_global_default_key2','keyring_vault7','true');"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT * FROM t1;"

echo "Change Global Key Provider to kmip"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_add_global_key_provider_kmip('keyring_kmip7','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_key_jane_doe.pem');"
echo "Rotate the Global Default Principal Key"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_set_default_key_using_global_key_provider('my_global_default_key3','keyring_kmip7','true');"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT * FROM t1;"

echo "Change Global Key Provider to file"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_add_global_key_provider_file('keyring_file7','$data_dir/keyring.file');"
echo "Rotate the Global Default Principal Key"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT pg_tde_set_default_key_using_global_key_provider('my_global_default_key4','keyring_file7', 'true');"
$INSTALL_DIR/bin/psql  -d db7 -c"SELECT * FROM t1;"
...
$INSTALL_DIR/bin/psql  -d db8 -c"SELECT pg_tde_add_database_key_provider_vault_v2('keyring_vault','$token','$vault_url','$secret_mount_point','$vault_ca');"
...
echo "Change Key provider to kmip"
$INSTALL_DIR/bin/psql  -d db8_new -c"SELECT pg_tde_add_database_key_provider_kmip('keyring_kmip','0.0.0.0',5696,'/tmp/certs/root_certificate.pem','/tmp/certs/client_certificate_jane_doe.pem','/tmp/certs/client_key_jane_doe.pem');"
...
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_vault_v2('vault_keyring9','$token','$vault_url','$secret_mount_point','$vault_ca');"
...
$INSTALL_DIR/bin/psql  -d test9 -c"SELECT pg_tde_set_key_using_global_key_provider('vault_key92','vault_keyring9','true');"

for i in $(seq 1 100); do
# change encryption key, uing default key
$INSTALL_DIR/bin/psql  -d test9 -c"SELECT pg_tde_set_key_using_database_key_provider('file_key9','keyring_file9');"
...
echo "##################################################################################################"
echo "..Add a global key provider"
$INSTALL_DIR/bin/psql -d postgres -c"SELECT pg_tde_add_global_key_provider_vault_v2('vault_keyring10','$token','$vault_url','$secret_mount_point','$vault_ca');"
...
$INSTALL_DIR/bin/psql -d test10 -c"SELECT pg_tde_set_key_using_global_key_provider('vault_key10','vault_keyring10','true');"
...
echo "############################################################################################"
echo "..Add a global key provider"
$INSTALL_DIR/bin/psql -d postgres -c"SELECT pg_tde_add_global_key_provider_vault_v2('vault_keyring11','$token','$vault_url','$secret_mount_point','$vault_ca');"

echo "..Create Global server Key"
$INSTALL_DIR/bin/psql -d postgres -c"SELECT pg_tde_set_server_key_using_global_key_provider('principal_key','vault_keyring11')"
...
echo "..Add a global key provider"
$INSTALL_DIR/bin/psql  -d postgres -c"SELECT pg_tde_add_global_key_provider_vault_v2('vault_keyring12','$token','$vault_url','$secret_mount_point','$vault_ca');"
echo "..Set a default key for encryption"
...
$INSTALL_DIR/bin/psql  -d postgres -c"CREATE DATABASE test12;"
$INSTALL_DIR/bin/psql  -d test12 -c"CREATE EXTENSION pg_tde;"
...
echo "Delete the old key provider. Note old keys are still available. Roadmap: Implement pg_tde_set_delete_key()"
$INSTALL_DIR/bin/psql  -d test1 -c"SELECT pg_tde_delete_global_key_provider('vault_keyring12');"
