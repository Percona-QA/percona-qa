...
# --- WAL Cleanup right after pg_receivewal startup ---
latest_wal=$(ls -1 "$ARCHIVE_DIR" | grep -v "\.partial$" | grep -v "\.log$" | sort | tail -n 1)
if [ -n "$latest_wal" ]; then
    echo "Running pg_archivecleanup on WALs before: $latest_wal"
    "$INSTALL_DIR/bin/pg_archivecleanup" "$ARCHIVE_DIR" "$latest_wal" >> "$ARCHIVE_DIR/pg_archivecleanup.log" 2>&1
else
    echo "No complete WAL file found for cleanup yet"
fi
...
for i in {1..4}; do
    ...
    sed -i '/archive_cleanup_command/d' "$PGDATA/postgresql.conf"
    ...
    echo "archive_cleanup_command = '$INSTALL_DIR/bin/pg_archivecleanup $ARCHIVE_DIR %r'" >> "$PGDATA/postgresql.conf"
