...
MS57_TAR=`ls -1td mysql-5.7* | grep ".tar" | head -n1` || true
PS57_TAR=`ls -1td ?ercona-?erver-5.7* | grep ".tar" | head -n1` || true
MS57_11_TAR=`ls -1td mysql-5.7.11* | grep ".tar" | head -n1` ||  true
...
if [ ! -z $MS57_TAR ];then
  ...
  wget http://downloads.mysql.com/archives/get/file/mysql-test-5.7.10-linux-glibc2.5-x86_64.tar.gz
  wget http://downloads.mysql.com/archives/get/file/mysql-5.7.10-linux-glibc2.5-x86_64.tar.gz
  MS_TEST_TAR=`ls -1td mysql-test-5.7* | grep -v ".tar" | head -n1`
  MS57_TAR=`ls -1td mysql-5.7* | grep ".tar" | head -n1`
  tar -xzf $MS57_TAR
  tar -xzf $MS_TEST_TAR
  MS57_BASE=`ls -1td mysql-5.7* | grep -v ".tar" | head -n1`
  export PATH="$ROOT_FS/$MS57_BASE/bin:$PATH"
...
if [ ! -z $MS57_11_TAR ];then
  tar -xzf $MS57_11_TAR
  MS57_11_BASE=`ls -1td mysql-5.7.11* | grep -v ".tar" | head -n1`
  export PATH="$ROOT_FS/$MS57_11_BASE/bin:$PATH"
else
  ...
  MS57_11_TAR=`ls -1td mysql-5.7.11* | grep ".tar" | head -n1`
  tar -xzf $MS57_11_TAR
  tar -xzf $MS_TEST_TAR
  MS57_11_BASE=`ls -1td mysql-5.7* | grep -v ".tar" | head -n1`
  export PATH="$ROOT_FS/$MS57_11_BASE/bin:$PATH"
fi
...
MS57_11_BASEDIR="${ROOT_FS}/$MS57_11_BASE"
...
function create_emp_db()
{
  ...
  cat $ROOT_FS/test_db/$SQL_FILE \
   ...
   $MS57_BASEDIR/bin/mysql --socket=$SOCKET -u root < ${ROOT_FS}/test_db/${DB_NAME}_${SE_NAME}.sql || true
   popd
}
...
$SBENCH --test=$LPATH/parallel_prepare.lua --report-interval=10 --mysql-engine-trx=yes --mysql-table-engine=innodb --oltp-table-size=$TSIZE --oltp_tables_count=$TCOUNT --mysql-db=test --mysql-user=root  --num-threads=$NUMT --db-driver=mysql --mysql-socket=$WORKDIR/ms57.sock prepare  2>&1 | tee $WORKDIR/logs/sysbench_prepare.txt
...
create_emp_db employee_1 innodb employees.sql $WORKDIR/ms57.sock

echoit "Loading employees partitioned database with innodb engine.."
create_emp_db employee_2 innodb employees_partitioned.sql $WORKDIR/ms57.sock

echoit "Loading employees database with myisam engine.."
create_emp_db employee_3 myisam employees.sql $WORKDIR/ms57.sock
...
echoit "Loading sakila test database"
$MS57_BASEDIR/bin/mysql --socket=$WORKDIR/ps57.sock -u root < ${SCRIPT_PWD}/sample_db/sakila.sql

echoit "Loading world test database"
$MS57_BASEDIR/bin/mysql --socket=$WORKDIR/ps57.sock -u root < ${SCRIPT_PWD}/sample_db/world.sql
...
echoit "Version info after cross upgrade (MS-5.7>PS-5.7)"
...
pushd ${MS57_11_BASEDIR}/mysql-test/
...
$MS57_11_BASEDIR/bin/mysql_upgrade -S $WORKDIR/ms57.sock -u root --force 2>&1 | tee $WORKDIR/logs/ms_mysql_upgrade.log

echoit "Loading sakila test database"
$MS57_11_BASEDIR/bin/mysql --socket=$WORKDIR/ms57.sock -u root < ${SCRIPT_PWD}/sample_db/sakila.sql

echoit "Loading world test database"
$MS57_11_BASEDIR/bin/mysql --socket=$WORKDIR/ms57.sock -u root < ${SCRIPT_PWD}/sample_db/world.sql
...
$MS57_11_BASEDIR/bin/mysql -S $WORKDIR/ms57.sock  -u root -e "select @@version,@@version_comment;"

$MS57_11_BASEDIR/bin/mysqladmin  --socket=$WORKDIR/ms57.sock -u root shutdown
