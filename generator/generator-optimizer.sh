...
partse()     { if [ $[$RANDOM % 20 + 1] -le 3  ]; then echo "`storage` ENGINE`equals`${ENGINE1}" ; fi }  # 15% ENGINE (for partitioning)
...
query(){
  ...
  case $[$RANDOM % 29 + 1] in
    ...
    [1-3]) case $[$RANDOM % 6 + 1] in  # CREATE (needs further work)
        1) echo "CREATE `temp` TABLE `ifnotexist` `table` (c1 `pk`,c2 `ctype`,c3 `ctype`) ENGINE=${ENGINE1} `partitionby`";;
        2) echo "CREATE `temp` TABLE `ifnotexist` `table` (c1 `ctype`,c2 `ctype`,c3 `ctype`) ENGINE=${ENGINE1} `partitionby`";;
        3) C1TYPE=`ctype`
           if [ "`echo ${C1TYPE} | grep -o 'CHAR'`" == "CHAR" -o "`echo ${C1TYPE} | grep -o 'BLOB'`" == "BLOB" -o "`echo ${C1TYPE} | grep -o 'TEXT'`" == "TEXT" ]; then
             echo "CREATE `temp` TABLE `ifnotexist` `table` (c1 ${C1TYPE},c2 `ctype`,c3 `ctype`, PRIMARY KEY(c1(`n10`))) ENGINE=${ENGINE1} `partitionby`"
           else
             echo "CREATE `temp` TABLE `ifnotexist` `table` (c1 ${C1TYPE},c2 `ctype`,c3 `ctype`, PRIMARY KEY(c1)) ENGINE=${ENGINE1} `partitionby`"
           fi;;
    ...
    8) case $[$RANDOM % 2 + 1] in  # Load data infile/select into outfile (needs further work)
    ...
    9) case $[$RANDOM % 7 + 1] in  # Select (needs further work)
        1) echo "`selectq`";;
    ...
    10) case $[$RANDOM % 9 + 1] in  # Delete (complete, except join clauses could be extended with `joinlron` and `joinlronla` as above)
    ...
    11) case $[$RANDOM % 3 + 1] in  # UPDATE (needs further work)
    ...
    12) echo "SET `emglobses` TRANSACTION ISOLATION LEVEL `isolation`";;  # Transaction isolation level (complete)
    1[3-4]) case $[$RANDOM % 37 + 1] in  # SET statements (complete)
       ...
       3[5-7]) case $[$RANDOM % 3 + 1] in  # A few selected (more commonly needed, except the first) SET statements that get a higher frequency then the ones above
             1) echo "SET @@`globses`.tx_read_only=0";;  # Ensure tx_read_only regularly set back to off
             2) echo "SET @@GLOBAL.read_only=0";;        # Idem, for read_only
             3) echo "SET @@GLOBAL.super_read_only=0";;  # Idem, for super_read_only
    ...
    15) case $[$RANDOM % 7 + 1] in  # Alter (needs few other additions like table types etc)
        1) echo "ALTER TABLE `table` ADD COLUMN c4 `ctype`";;
        2) echo "ALTER TABLE `table` DROP COLUMN c`n3`";;
        3) echo "ALTER TABLE `table` DROP PRIMARY KEY";;
        4) echo "ALTER TABLE `table` ADD INDEX (c`n3`)";;
        5) echo "ALTER TABLE `table` ADD UNIQUE (c`n3`)";;
        6) echo "ALTER TABLE `table` ADD INDEX (c`n3`), ADD UNIQUE (c`n3`)";;
        7) echo "ALTER TABLE `table` MODIFY c`n3` `ctype`";;
        *) echo "Assert: invalid random case selection in ALTER case";;
       esac;;
    16) case $[$RANDOM % 7 + 1] in  # Nested `query` calls: items 1 and 2 need further work to not select all types of queries (needs sub-functions, for example select(){}
    ...
    17) case $[$RANDOM % 4 + 1] in  # Repair/optimize/analyze/rename table (complete)
    ...
    1[8-9]) case $[$RANDOM % 7 + 1] in  # Transactions (complete, except review/add in different section?; http://dev.mysql.com/doc/refman/5.7/en/begin-end.html)
    ...
    20) case $[$RANDOM % 19 + 1] in  # Lock tables (complete)
   ...
   1[5-9]) echo "LOCK TABLES `table` `asalias3` `locktype`, `table` AS a`n9` `locktype`, `table` AS a`n9` `locktype`";;  # Locking more tables leads to a higher overall number of queries that succeed
    ...
    21) case $[$RANDOM % 3 + 1] in  # Savepoints (complete)
    ...
    22) case $[$RANDOM % 7 + 1] in  # Calling & setup of functions and procedures (complete)
    ...
    23) case $[$RANDOM % 4 + 1] in  # Numeric functions: this should really become a callable function so that it can be used instead of `data` for example, etc.
    ...
    # RV-HIGH LOAD TESTER
    2[4-9]) echo "SET @@`globses`.OPTIMIZER_SWITCH=\"`optsw`=`onoff`\"";;
